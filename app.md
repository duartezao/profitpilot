...
* **Taxas EU por categoria (Shopify)** — nos modos `shopify` e `variant`, painel em `/cogs` para registar o **total diário** da taxa de 3 € por categoria por encomenda (vendas UE). Coleção `euCategoryFeeDays`. Soma ao COGS do dia no lucro e na % de COGS. Podes guardar **0** se não houve taxa ou já está incluída no custo do produto.
