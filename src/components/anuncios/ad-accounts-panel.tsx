...
export function AdAccountsPanel({
  ...
  const metaOAuthStart = hrefWithScope(
    `/api/oauth/meta/start?store=${encodeURIComponent(storeId)}`,
    searchParams,
  );
  ...
  useEffect(() => {
    ...
    if (err) {
      setOauthMsg(
        err === "cancelled"
          ? "Ligação Meta cancelada."
          : `OAuth Meta falhou: ${err}`,
      );
      return;
    }
    if (searchParams.get("oauth") !== "meta" || !canEdit) return;
    void consumeMetaOAuthTokenAction().then((t) => {
      if (t) {
        setPlatform("meta");
        setToken(t);
        setOauthMsg("Token Meta recebido — clica em Procurar contas.");
      }
    });
  ...
  function runDiscover() {
    ...
    setSelectedId("");
    startDiscover(async () => {
      const res = await discoverAdAccountsAction(platform, token);
      if (res.error) {
  ...
  return (
    <CollapsibleSection
      ...
      description="Meta, Google e TikTok — sync automático do gasto de hoje."
      badge={
        accounts.length > 0 ? (
          <span
    ...
    >
      {canEdit && (
        <CollapsibleSection
          title="Ligar conta"
          description="Escolhe a plataforma, cola o token e procura contas."
        >
