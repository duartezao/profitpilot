...
export function AdAccountFeesForm({
  ...
  useActionOkOnce(state.ok, onSaved);
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            min={0}
            step="0.01"
            key={`fixed-${account.id}-${account.apiExtraFeeFixed}`}
        ...
        <div>
          ...
          <input
            ...
            max={100}
            step="0.1"
            key={`pct-${account.id}-${account.apiAgencyFeePercent}`}
      ...
      {state.ok && <p className="text-xs text-positive">Fees guardadas.</p>}
      <button
        type="submit"
        disabled={pending}
